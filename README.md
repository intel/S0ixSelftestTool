...
2. If the users sees _"awk: line 10: function gensub never defined"_ message during
 running the script, please install `gawk`.

3. If the users sees _"sudo: xxd: command not found"_ message during running the script,
please try to install a `xxd` (hex dump utility) or `vim-common` package.
